...
methods
  ...
  function x2 = leftact(g, x)
      ...
      if ( (size(x,1) == 3) && (size(x,2) == 1) ) % If three vector.
          x2 = g.M(1:3,:)*[x;ones(1,size(x,2))]; % treat like a point.
      elseif ( (size(x,1) == 4) && (size(x,2) == 1) ) % else it is homogeneous.
...
methods(Static)
  ...
  if(wmag == 0) %handle divide by 0 if omega is 0
      Rexp = eye(3);
      Texp = v * tau;
  end
